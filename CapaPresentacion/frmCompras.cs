...
namespace CapaPresentacion
{
    public partial class frmCompras : Form
    {
        ...
        private void txtdcoProveedor_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyData == Keys.Enter)
            {

                Proveedor oProve = new CN_Proveedor().Listar().Where(p => p.Documento == txtdcoProveedor.Text && p.Estado == true).FirstOrDefault();

                if (oProve != null)
                {
                    txtdcoProveedor.BackColor = Color.Honeydew;
                    txtidProveedor.Text = oProve.IdProveedor.ToString();
                    txtRazSocProveedor.Text = oProve.RazonSocial;

                }
                else
                {
                    txtdcoProveedor.BackColor = Color.MistyRose;
                    txtidProveedor.Text = "0";
                    txtRazSocProveedor.Text = "";
